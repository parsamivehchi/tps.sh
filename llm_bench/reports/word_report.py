...
def generate_word_report(run_id: str | None = None) -> Path:
    """Generate the polished tps.sh Word report."""
    doc = Document()
    ...
    _add_body(doc,
        "tps.sh is a benchmarking toolkit that systematically compares local open-source "
    ...
    run = footer.add_run("Generated by tps.sh — github.com/parsamivehchi/tps.sh")
    run.font.size = Pt(8)
    run.font.color.rgb = LIGHT_TEXT
